...
export default class Bots {
    ...
    async deleteCustomBot(user_id, bot_id) {
        if (!bot_id || !user_id) {
            console.error("bot_id and user_id are required");
            return null;
        }
        try {
            const response = await fetch(`${BASE_URL}/custom_bots/del_bot/${user_id}/${bot_id}`, {
                method: "DELETE",
            });

            if (response.status === 204) {
                console.log(`Custom Bot ${bot_id} deleted successfully.`);
                return true;
            } else {
                console.error(`Failed to delete Custom Bot ${bot_id}:`, data.error);
                return null;
            }
        } catch (error) {
            console.error("Network error while deleting custom bot:", error);
            return null;
        }
    }
}